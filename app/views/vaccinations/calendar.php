        ...
        <div class="col-12">
            <div class="card dashboard-card">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            <label for="calendarAnimalFilter" class="form-label">Filter by Animal</label>
                            <select class="form-control" id="calendarAnimalFilter">
                                <option value="">All Animals</option>
                                <?php foreach ($animals as $animal): ?>
                                <option value="<?php echo $animal['animal_id']; ?>"><?php echo $animal['name']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-4">
...
var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    eventDidMount: function(info) {
        ...
        const tooltip = [
            `${event.title}`,
            `Animal: ${event.extendedProps.animal_name}`,
            `Date: ${event.start.toLocaleDateString()}`,
            `Status: ${event.extendedProps.status}`,
            `Type: ${event.extendedProps.vaccine_type || 'N/A'}`
        ].join('\n');
    ...
    const animalFilter = document.getElementById('calendarAnimalFilter');
    ...
    function applyFilters() {
        const animalId = animalFilter.value;
        const status = statusFilter.value;

        calendar.getEvents().forEach(function(event) {
            let showEvent = true;

            if (animalId && event.extendedProps.animalId !== animalId) {
                showEvent = false;
            }
